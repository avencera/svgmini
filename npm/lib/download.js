...
const tmpDir = path.join(os.tmpdir(), `svgmini-cache-${packageVersion}`);
...
async function unzipsvgmini(zipPath, destinationDir) {
  if (isWindows) {
  ...
  const expectedName = path.join(destinationDir, "svgmini");
  ...
  throw new Error(
    `Expecting svgmini or svgmini.exe unzipped into ${destinationDir}, didn't find one.`
  );
}
...
module.exports = async opts => {
  ...
  const assetName =
    ["svgmini", opts.version, opts.target].join("-") + extension;
  ...
  try {
    const destinationPath = await unzipsvgmini(assetDownloadPath, opts.destDir);
    if (!isWindows) {
